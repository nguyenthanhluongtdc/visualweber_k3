...
BACKUP_PATH_PROJECT="$(date +"%d.%m.%y")_kianewcerato-k3"
...
BACKUP_SOURCENAME=kianewcerato-k3_source-$(date +"%Y-%m-%d_%H.%I.%S").zip
BACKUP_SQLNAME=kianewcerato-k3_dbs-$(date +"%Y-%m-%d_%H.%I.%S").sql
...
(cd $SCRIPT_PATH/../ && $MYSQLDUMP -uuserdb.dev.kianewcerato_k3 -p0YUg3115FH67I5jO --default-character-set utf8 dev_kianewcerato_k3 > $BACKUP_SQLNAME)
...
## echo -e "$VERT--> http://kianewcerato-k3.dev.gistensal.com/$BACKUP_SOURCENAME $NORMAL"
## echo -e "$VERT--> http:/kianewcerato-k3.dev.gistensal.com/$BACKUP_SQLNAME $NORMAL"
